...
movesnake = (movecamera = 1) => {
  ...
  for(i = 0; i < snakelength; i++){
    window["snakecubemove" + i].style.transform = "translateX(" + (snakepos[i][0] * 10 + 1) + "vmin) translateY(" + (snakepos[i][1] * 10 + 1) + "vmin) translateZ(" + (snakepos[i][2] * 10 + .5) + "vmin) " + (!mobile && snakepos[i][2] > 0 ? "rotateX(-90deg) translateY(-5vmin) translateZ(-4vmin)" : "");
